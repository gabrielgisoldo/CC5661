...
def main():
	...
	while(True):
		...
		if resp['problema']['tipo'] == 'ordenacao':
			...
			mergeSort(resp['problema']['elementos'], 0 ,resp['problema']['n']-1)
			json_resp = json.dumps(resp) #parser para JSON
		
			requests.post('http://localhost:5000/solucao', data=json_resp) #retorna

		elif resp['problema']['tipo'] == 'multiplicacao_matrizes':
			try:
				resp['problema']['solucao'] = multiplicar_matrizes(resp['problema']['matrizes'])
			except IndexError:
				resp['problema']['solucao'] = []
			requests.post('http://localhost:5000/solucao', data=json.dumps(resp))
